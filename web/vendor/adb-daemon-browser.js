...
//
// Important UX detail:
// - If the pending key is persisted only after a successful CNXN, then any
//   failure happening *after* the user accepted the RSA dialog (or any
//   transport/packet error) causes the next reconnect to generate a brand
//   new key => a new fingerprint => the phone asks again.
//
// To keep fingerprint stable across reconnect attempts, we persist the
// generated key right after sending the PUBKEY. We still try to finish the
// handshake; if it fails, reusing the stored key on the next attempt should
// avoid another phone confirmation (because the phone already trusts that
// public key).
const KeyStore = {
    async iterateKeys() {
    ...
    async commitPendingKey(pending) {
        try {
            ...
            const incoming = pending?._storageData;
            if (!incoming) return;
            const incomingPub = incoming.publicKey;
            // Dedupe by publicKey to avoid bloat if we commit on retries.
            if (keys.some(k => k && k.publicKey === incomingPub)) return;
            keys.push(incoming);
            // Keep it bounded (should be very small in normal use).
            while (keys.length > 5) keys.shift();
            localStorage.setItem('adb_keys', JSON.stringify(keys));
        } catch (e) {
            console.warn('KeyStore.commitPendingKey failed:', e);
        }
    },
};
...
async function authenticate(transport, firstPacket) {
    ...
    // Persist immediately to keep fingerprint stable across reconnect attempts.
    // This mirrors how `private.pem` is treated: once we have a key, we keep it.
    await KeyStore.commitPendingKey(pending);
