...
const KeyStore = {
    ...
    async generateKey() {
        ...
        keys.push({ privateKey: btoa(bin), publicKey: new TextDecoder().decode(pubPayload) });
        localStorage.setItem('adb_keys', JSON.stringify(keys));
        return { privateKey: pair.privateKey, publicKeyPayload: pubPayload };
    },
};
    ...
    async readBytes(count, timeoutMs = 15000) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                this.pendingReads = this.pendingReads.filter(r => r.resolve !== resolve);
                reject(new Error('WS read timeout'));
            }, timeoutMs);
            this.pendingReads.push({ count, resolve, reject, timer });
        });
    }
    async close() {
...
// Same feature list the ADB server sends to a device (server-exact bytes
// proven to be accepted by this phone's adbd over TCP/WebSocket; the
// minimal list + trailing ';' was silently ignored by the TCP listener).
const CNXN_FEATURES = 'shell_v2,cmd,stat_v2,ls_v2,fixed_push_mkdir,apex,abb,fixed_push_symlink_timestamp,abb_exec,remount_shell,track_app,sendrecv_v2,sendrecv_v2_brotli,sendrecv_v2_lz4,sendrecv_v2_zstd,sendrecv_v2_dry_run_send,devicetracker_proto_format,devraw,app_info,server_status,track_mdns';

// AUTH handshake: token → signatures with known keys → public key.
async function authenticate(transport, firstPacket) {
    let packet = firstPacket || await readPacket(transport);
    ...
    for (const key of keys) {
        ...
        packet = await readPacket(transport);
    ...
    const fresh = await KeyStore.generateKey();
    await sendPacket(transport, ADB.CMD_AUTH, ADB.AUTH_RSAPUBLICKEY, 0, fresh.publicKeyPayload);
    packet = await readPacket(transport);
    if (packet.cmd === ADB.CMD_CNXN) return packet.payload;
